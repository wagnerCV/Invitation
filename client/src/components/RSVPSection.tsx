  ...
  const handleSubmit = async () => {
    ...
    try {
      await saveRSVP({
        ...
        going_to_reception: formData.attending === "yes", // Simplified logic for now
        dietary_restrictions: formData.dietary.join(", "),
  ...
  const toggleDietary = (restriction: string) => {
    setFormData((prev) => {
      ...
      if (current.includes(restriction)) {
        return { ...prev, dietary: current.filter((r) => r !== restriction) };
      } else {
        return { ...prev, dietary: [...current, restriction] };
      }
    });
  };

  return (
    <section className="py-24 bg-sand/20 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 border border-sand/30 min-h-[400px] flex flex-col justify-center relative overflow-hidden">
          {/* Progress Bar */}
          {step < 4 && (
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["Vegetariano", "Vegano", "Sem Glúten", "Sem Lactose"].map((diet) => (
                          <button
                            key={diet}
                            ...
                            className={`px-4 py-2 rounded-full text-sm transition-colors border ${
                              formData.dietary.includes(diet)
                                ...
                                : "bg-white text-soft-black border-sand hover:border-emerald"
                            }`}
                          >
                            {diet}
                          </button>
